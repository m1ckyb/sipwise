...
### Fixed
- Enabled Row-Level Security (RLS) on `public.rate_limits` and `public.sipwise_token_blacklist` tables to resolve `rls_disabled_in_public` and `sensitive_columns_exposed` security vulnerabilities.
